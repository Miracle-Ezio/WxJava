...
**总成本（首年）**：约 ~5000 元（含一年服务器 + 备案 + 短信预付 + 小程序认证 + 域名）。
...
| 主体 | 北京思达芮医疗美容诊所有限公司 |
| 类目 | 医疗 → 医美整形（需上传**医疗机构执业许可证**） |
| 凭证 | 营业执照、法人身份证、对公账户验证 |
| 费用 | 300 元/年企业认证 |
| 审核时间 | 1–3 天（医美类目可能更长） |
...
| 到店前 24h 提醒 | 同上 |
| 到店前 2h 提醒 | 同上 |
| 规划方案推送 | thing1（方案名）、thing2（提示）、date3（推送时间） |

每个 1–3 天审核。通过后拿到模板 ID（形如 `9Bae_xxxx`），后面填配置。
...
| 桶配置 | **私有读写**（医美照片合规要求）、地域选 ap-beijing（北京） |
